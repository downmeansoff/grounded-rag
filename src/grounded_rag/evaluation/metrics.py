...
Считать «сколько чанков нужного тендера попало в топ-5» можно только там, где
ответ заведомо лежит в одном документе. На запросе «кто заказчик услуг
гардероба» правильных тендеров четырнадцать, и это число не значит ничего.
...
def normalize(text: str) -> str:
    ...
    В документах закупки таблицы приезжают с неровными отступами, и один и тот
    же фрагмент в разметке и в чанке отличается только пробелами. Регистр не
