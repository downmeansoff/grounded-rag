...
class Settings(BaseSettings):
    ...
    # Под что подогнан движок: чем разбирается исходный файл и что говорится
    # модели при генерации контекста и ответа. tenders это формат выгрузки
    # tenderhunt, plain это любая папка с .txt и .md. Смена профиля меняет
    # разбор документов и ключ кэша контекстов, то есть требует пересбора
    # индекса, а не только правки этой строки.
    domain: str = "tenders"
