...
@dataclass(frozen=True)
class Question:
    """Вопрос набора. Фильтр по метаданным это часть вопроса, а не настройка прогона.

    Пары ключ-значение, а не словарь, чтобы вопрос остался неизменяемым: набор
    читается один раз и гоняется в трёх режимах подряд, и правка фильтра между
    режимами сделала бы числа несравнимыми.
    """
